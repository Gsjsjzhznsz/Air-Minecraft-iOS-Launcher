 ...
 */
// Amethyst iOS overlay (wins the lwjgl-341.jar merge): verbatim LWJGL 3.4.1
// ThreadLocalUtil. The versioned jar shipped a patched copy whose JNI native
// was renamed to "nsetupEnvData", bridged through the launcher binary
// (input_bridge_v3.m) — but the SDL3 launch path never registers that binary
// with the JVM (UIKit's System.load is only reached via the -jar branch), so
// HotSpot's classloader-scoped lookup threw
// UnsatisfiedLinkError: nsetupEnvData at GL.createCapabilities and the
// OpenGL backend crashed on MC 26.3. The prebuilt liblwjgl.dylib exports the
// stock name Java_org_lwjgl_system_ThreadLocalUtil_setupEnvData, so binding
// the stock declaration removes the bridge from the dependency chain entirely.
