...
- (void)buildFaqData {
    ...
    fpsUnlock.answer = @"• 帧率上限：游戏内 视频设置 → 最大帧率（也可在启动器 Java 参数里看到 -Dmax.fps 解锁层）。设为\"无限制\"或高数值即靠上限；\n"
                       ...
                       @"• 26.x 整合包加载中/不操作时被压在 30fps（曾误以为 FSR 问题）：MC 26.3 新增“不活动限帧”——默认 AFK 模式下 60 秒无输入自动降到 30fps（10 分钟降到 10fps），整合包加载期无触摸正好触发。启动器已修复（Task104）：写入 inactivityFpsLimit=minimized + 每阻 45s 注入零增量滚轮心跳重置计时；日志锚点 [InputDiag] Task104 AFK heartbeat。若修复后重负载游玩仍只有 ~30fps，那是真实 GPU 负载（FSR 档位调低/降视距即可）；\n"
    ...
    fsrCorner.answer = @"典型表现：选了 Zink 渲染器并开启 FSR 超分档位后，游戏能正常进入与操作，但整个画面只占屏幕左下约三分之二，其余区域空白。\n\n"
                     ...
                     @"启动器已修复（Task99 探测 + Task100 权威呈现 + Task103 地面真值闭环 + Task104 远角哨兵）：上屏不再依赖驱动的 glFinish 回读——每帧由启动器自己从帧缓冲直读权威画面并独立上屏。哨兵验证升级为双角落（Task104）：左下角哨兵只能证明“角落里有升采样片元”，证不了全幅覆盖；右下角对角哨兵（屏幕右上）是全幅光栅化的必经之地——双哨兵全部命中＝升采样真实全幅落地；任一缺失＝立即改把游戏原始画面交 CoreAnimation 拉伸到全屏（双线性滤质的拉伸，几何恒全屏）。验证方法：日志搜 “[OSMBridge] Task103 EASU sentinel verdict”——LANDED 表示双哨兵全中、EASU 全幅上屏；NOT LANDED 表示已启用拉伸兜底（画面即刻全屏）；far=N/M 为远角哨兵命中率，[OSMBridge] Task104 EASU viewport check 一行可见驱动实际持有的视口尺寸。\n\n"
