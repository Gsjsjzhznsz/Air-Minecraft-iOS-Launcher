...
void load_libs() {
...
#else
    // iOS: 与 Amethyst 中 MobileGlues 相同的处理方式。
    // libdesktopglues.dylib 在编译时已链接 libEGL.framework / libGLESv2.framework,
    // 但运行时由 Java 端通过 dlopen(RTLD_LOCAL) 加载本库, 导致 ANGLE 框架符号
    // 不在全局作用域内, dlsym(RTLD_NEXT, ...) 等调用会返回 NULL, 进而在
    // init_target_egl 中引发空指针解引用崩溃 (SIGSEGV pc=0x0)。
    // 修复: 通过 dladdr 定位本库所在目录, 显式以 RTLD_GLOBAL 打开框架二进制,
    // 将其符号提升到全局作用域, 使 dlsym 能正常解析。
    Dl_info info;
    ...
    // 回退: 若显式 dlopen 失败 (如路径推断出错), 仍尝试 RTLD_NEXT (旧行为)
