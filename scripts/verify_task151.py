...
def main():
    ...
    for fn in sorted(os.listdir(os.path.join(REPO, "scripts"))):
        if not (fn.startswith("verify_task") and fn.endswith(".py")) or fn == "verify_task151.py":
            continue
        s = rd(f"scripts/{fn}")
        # Task168：l10n 基线 1952 -> 1954（净增 background.cards.neumorph.title），
    ...
    check("H no stale l10n anchors (expect 1955 everywhere (Task178 re-anchor), Task168 baseline)", not stale, str(stale))
