...
wiring / G l10n (4x1946, bing keys) + Info.plist / H re-anchored gates clean.
"""
import os
import re
import subprocess
...
def main():
    ...
    check("G gated baseline 1946 x4", len(set(counts.values())) == 1 and
          counts["en"] == 1946, str(counts))
    for lang in ["ja", "km"]:
        s = rd(f"Natives/resources/{lang}.lproj/Localizable.strings")
        ks = set(re.findall(r'^"([^"]+)"\s*=', s, re.M))
        missing = [k for k in bing_keys if k not in ks]
    ...
    check("H no stale l10n anchors (expect 1946 everywhere)", not stale, str(stale))
