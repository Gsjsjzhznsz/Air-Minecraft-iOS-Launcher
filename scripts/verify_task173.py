...
check("C3 灰化反转：开关开启时三行旧选项变灰关交互；滑条随开关启停",
      bsvc.count("cell.contentView.alpha = neumorphOn ? 0.35 : 1.0;") == 3
      and bsvc.count("cell.userInteractionEnabled = !neumorphOn;") == 3
...
check("E1 公告插入 index 2（server/task169 pin 不动；并行 task172-six 顺延 anns[3]，171/170/168 顺延 anns[4]/[5]/[6]）且 id 唯一",
      len(ids) == len(set(ids))
