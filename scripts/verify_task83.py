...
check("B12 FAQ：28 条目（Task83 19 + Task83b +3 + Task84 +1 Arm ASR + Task85 +1 替代方案调研 + Task86 +1 大型整合包首启卡死 + Task87 +1 LTW×26.x + Task94 +1 Sodium LWJGL 版本门 + Task95 +1 缺失 mod 闪退）",
      len(faq_items) == 28, f"got {len(faq_items)}")
...
check("B18 FAQ：设置路径与实际一致（视频设置分区，Task87 ltw26 后 3 处）",
      helpvc.count("设置 → 视频设置") == 3)
