...
check("C1 28 条目（Task86 +1，Task87 +1，Task94 +1，Task95 +1）", len(faq_items) == 28, f"got {len(faq_items)}")
...
check("C3 bigpack 注册进故障排除分类（stuck 之后，Task94 后 sodiumLwjgl、Task95 后 missingMods 殿后）",
      re.search(r"@\[ xray, greenFx, background, crash, stuck, bigpack, sodiumLwjgl, missingMods \]", helpvc) is not None)
