...
check("E1 30 条目（Task95 +missingMods，Task97 +cwdMismatch，Task98 +mc26sdl）", len(faq_items) == 30, f"got {len(faq_items)}")
...
check("E4 注册在故障排除分类（sodiumLwjgl 之后，cwdMismatch/mc26sdl 之前）",
      re.search(r"bigpack, sodiumLwjgl, missingMods, cwdMismatch, mc26sdl \]", helpvc) is not None)
...
cascade = {
    "scripts/verify_task83.py": "30 条目",
    "scripts/verify_task84.py": "len(faq_items) == 30",
    "scripts/verify_task85.py": "len(faq_items) == 30",
    "scripts/verify_task86.py": "len(faq_items) == 30",
    "scripts/verify_task87.py": "len(faq_items) == 30",
    "scripts/verify_task94.py": "len(faq_items) == 30",
}
for path, marker in cascade.items():
    content = read(path)
    check(f"F3 {os.path.basename(path)} FAQ 计数已 sync 30", marker in content)
